...
double** ReadMtxMatrix(int& n, int& m) {
    ...
    std::ifstream file("msc01050.mtx");
...
int main() {
    ...
    int iter = 0;
    while (diff > eps) {
#pragma omp parallel for shared(A, b, x, x_old) // распараллеливание цикла
        for (int i = 0; i < N; i++) {
    ...
    std::cout << "I'm Done! Time: " << finish << ". Iterations: " << iter << ". Norm = " << norm << std::endl;
